document.addEventListener("DOMContentLoaded", function () {
  ...
  miButton.addEventListener("click", async function (e) {
    e.preventDefault();
    loader(true);
    ...
    // Inyectar el script en la pestaña activa
...
function generarNombreAleatorio() {
  const index = Math.floor(Math.random() * nombrePersonas.length);
  return nombrePersonas[index].toLowerCase();
}
...
const inputsMonth = () => {
  ...
  inputMonth.forEach((month_input) => {
    ...
    let formattedMonth = month < 10 ? "0" + month : String(month);
    // Construir la fecha en formato YYYY-MM
    let date = year + "-" + formattedMonth;
...
const loader = async (valor) => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });

  // Inyectar el script en la pestaña activa para agregar/quitar la clase al body
  chrome.scripting.executeScript({
    target: { tabId: tab.id },
    function: (valor) => {
    ...
    args: [valor], // Usar el valor pasado a la función loader
  });
};
